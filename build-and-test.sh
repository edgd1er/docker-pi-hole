...
stop() {
	docker stop pipenv
	#update local pipfile if not in CI
	if [[ -z ${CI:-""} ]]; then
...
docker buildx build --load --platform=${PLATFORM} --build-arg alpine_version="3.19" --build-arg docker_version="24.0.7" --tag image_pipenv --file test/Dockerfile test/
docker run --rm \
