...
BR=$(git status | grep -oP "(?<=Sur la branche ).*" | grep -c "development")
if [[ 0 -eq ${BR} ]]; then
...
buildContainer() {
  ...
  while [[ 0 -eq $(docker compose -f docker-compose-v6.yml logs | grep -cP '(Container tag is|INFO: Blocking status is enabled)') ]]; do
    docker compose -f docker-compose-v6.yml logs | tail -5
...
checkDns() {
	testip=$(grep -A1 ports docker-compose-v6.yml | tail -1 | grep -oPm1 "(?<=- \")([0-9\.]+)")
