...
curl -L -s "https://github.com/just-containers/s6-overlay/releases/download/v${S6_OVERLAY_VERSION}/s6-overlay-noarch.tar.xz.sha256" -o noarch.sha256
sha256sum -c noarch.sha256 && tar Jxpf s6-overlay-noarch.tar.xz -C / || echo "s6-overlay-noarch.tar.xz sha256 mismatch"

curl -L -s "https://github.com/just-containers/s6-overlay/releases/download/v${S6_OVERLAY_VERSION}/s6-overlay-${S6_ARCH}.tar.xz" -o s6-overlay-${S6_ARCH}.tar.xz
curl -L -s "https://github.com/just-containers/s6-overlay/releases/download/v${S6_OVERLAY_VERSION}/s6-overlay-${S6_ARCH}.tar.xz.sha256" -o arch.sha256
sha256sum -c arch.sha256 && tar Jxpf s6-overlay-${S6_ARCH}.tar.xz -C / || echo "s6-overlay-${S6_ARCH}.tar.xz sha256 mismatch"
rm noarch.sha256 arch.sha256 s6-overlay-${S6_ARCH}.tar.xz s6-overlay-noarch.tar.xz
